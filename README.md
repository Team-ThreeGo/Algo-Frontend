# Algo-Frontend
# Algo

<img width="1799" height="544" alt="image" src="https://github.com/user-attachments/assets/5c2ff665-8728-4025-9dca-c1aa16d209f7" />

> 개발자 취업 준비생, 지금 우리에게 필요한 건? Algo!

<br>

## ThreeGo 팀원 소개

| <img src="https://prs.ohousecdn.com/apne2/content/community/v1-373521229070400.jpg?w=480&h=480&c=c&q=80&webp=1" width="200"/> | <img width="200" src="https://github.com/user-attachments/assets/5c088fb6-46f8-40fc-b746-5884800368c5" /> | <img src="https://item.kakaocdn.net/do/a7fd7c0630f8aea8419a565fb2773bbc8f324a0b9c48f77dbce3a43bd11ce785" width="200"/> | <img src="https://cdn.imweb.me/thumbnail/20250911/f5a190ad907b7.png" width="200"/> | <img src="https://i.namu.wiki/i/GQMqb8jtiqpCo6_US7jmWDO30KfPB2MMvbdURVub61Rs6ALKqbG-nUATj-wNk7bXXWIDjiLHJxWYkTELUgybkA.webp" width="200"/> |
|:----------------------------------------------------------------------------------------------------------------------------:|:--------------------------------------------------------------------------------------------------------------------------:|:------------------------------------------------------------------------------------------------------------------------:|:------------------------------------------------------------------------------------------------------------------------:|:------------------------------------------------------------------------------------------------------------------------------------------------:|
| [**강지륜**](https://github.com/rosieisor) | [**김민지**](https://github.com/ghi512) | [**배태용**](https://github.com/HoodRyan) | [**정유진**](https://github.com/YujinJeong1006) | [**허창훈**](https://github.com/huni2) |


## 목차



---


## 1. 프로젝트 소개

### 기술 요약
### 🌿 FrontEnd
![Vue.js](https://img.shields.io/badge/vuejs-%2335495e.svg?style=for-the-badge&logo=vuedotjs&logoColor=%234FC08D) ![Vite](https://img.shields.io/badge/vite-%23646CFF.svg?style=for-the-badge&logo=vite&logoColor=white) ![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E) ![Element](https://img.shields.io/badge/element-0DBD8B.svg?style=for-the-badge&logo=element&logoColor=white) ![NPM](https://img.shields.io/badge/NPM-%23CB3837.svg?style=for-the-badge&logo=npm&logoColor=white)

### 🗝️ Backend
<img src="https://img.shields.io/badge/SpringBoot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white"/> ![Spring](https://img.shields.io/badge/spring-%236DB33F.svg?style=for-the-badge&logo=spring&logoColor=white) ![JWT](https://img.shields.io/badge/JWT-black?style=for-the-badge&logo=JSON%20web%20tokens) <img src="https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white"/> <img src="https://img.shields.io/badge/JPA-59666C?style=for-the-badge&logo=hibernate&logoColor=white"/> <img src="https://img.shields.io/badge/MyBatis-000000?style=for-the-badge&logo=databricks&logoColor=white"/> ![Amazon S3](https://img.shields.io/badge/Amazon%20S3-FF9900?style=for-the-badge&logo=amazons3&logoColor=white)

### 🗂️ DB
<img src="https://img.shields.io/badge/MariaDB-003545?style=for-the-badge&logo=mariadb&logoColor=white"/> ![MySQL](https://img.shields.io/badge/mysql-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white)

### 💾 Server
<img src="https://img.shields.io/badge/Spring%20Cloud%20Gateway-339933?style=for-the-badge&logo=spring&logoColor=white"/> <img src="https://img.shields.io/badge/Eureka-4285F4?style=for-the-badge&logo=google-cloud&logoColor=white"/>

### 🔧 Tools
![Figma](https://img.shields.io/badge/figma-%23F24E1E.svg?style=for-the-badge&logo=figma&logoColor=white) ![GitHub](https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white) ![Visual Studio Code](https://img.shields.io/badge/Visual%20Studio%20Code-0078d7.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white) <img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white"/>

### 프로젝트 배경


### 주요 기능

<details>
<summary> 👤 &nbsp; 회원가입 및 로그인 </summary>
<div markdown=1>
  <details>
  <summary> &nbsp; 회원가입 </summary>
  <p align="center">
    <img src="https://github.com/user-attachments/assets/7aaf1e41-ebca-4962-9ea1-54ae26a3d015"     width="600">

  </p>
  </details>

  <details>
  <summary> &nbsp; 로그인 </summary>
  <p align="center">
    <img src="https://github.com/user-attachments/assets/1ecf697c-9465-4f7b-bc34-0feda3b77b63" 
      width="600">
  </p>
  </details>
</details>  

<details>
<summary> 📚 &nbsp; 알고리즘 개념 학습 </summary>
<div markdown=1>

<details>
  <summary> &nbsp; 알고리즘 로드맵 조회 </summary>
  <p align="center">
     <img src="https://github.com/user-attachments/assets/0cac4cd3-8f18-45d3-a4d3-c78ef9ae61e5" width="600">
  </p>
  </details>

  <details>
  <summary> &nbsp; 알고리즘 학습 게시물 조회 / 추천하기 / 대댓글 등록 수정 삭제 </summary>
  <p align="center">
     <img src="https://github.com/user-attachments/assets/76ba3141-a62b-4efe-9f60-b9876cab76e6" width="600">
  </p>
  </details>

  <details>
  <summary> &nbsp; 알고리즘 퀴즈 풀이 </summary>
  <p align="center">
     <img src="https://github.com/user-attachments/assets/ff921692-c108-4ef4-8375-45e633f3d242" width="600">
  </p>
  </details>

</details>

<details>
<summary> 🤖 &nbsp; 코딩 풀이 및 AI 피드백 제공 </summary>
<div markdown=1>

<details>
  <summary> &nbsp; 코딩 문제 목록 조회 </summary>
  <p align="center">
     <img src="https://github.com/user-attachments/assets/b43b6531-fcf9-42d0-9766-2a514f8d1952" width="600">
  </p>
  </details>

  <details>
  <summary> &nbsp; 풀이글 등록, 상세 조회 </summary>
  <p align="center">
     <img src="https://github.com/user-attachments/assets/0cdac0b1-c3b4-4427-b1bd-f7d54ed820de" width="600">
  </p>
  </details>

</details>


<details>
<summary> 🏢 &nbsp; 기업별 정보 공유 </summary>
<div markdown=1>

<details>
  <summary> &nbsp; 목록, 상세 조회 </summary>
  <p align="center">
     <img src="https://github.com/user-attachments/assets/d41f830d-a278-4895-8bef-f6a08e9a815a" width="600">
  </p>
  </details>

<details>
  <summary> &nbsp; 공유글 작성 </summary>
  <p align="center">
     <img src="https://github.com/user-attachments/assets/9ffcc400-2dbe-4e8f-ad94-2b52fbdbb7a7" width="600">
  </p>
  </details>

<details>
  <summary> &nbsp; 관리자 - 승인 </summary>
  <p align="center">
    <img src="https://github.com/user-attachments/assets/4793a72c-db58-474c-9c72-0af6eb86bb57" width="600">
  </p>
  </details>

  <details>
  <summary> &nbsp; 관리자 - 반려 </summary>
  <p align="center">
        <img src="https://github.com/user-attachments/assets/bc2edfe3-6323-456c-95b1-50f83dad43fa" width="600">
  </p>
  </details>

</details>


<details>
<summary> 📖 &nbsp; 스터디 </summary>
<div markdown=1>

  <details>
  <summary> &nbsp; 스터디 모집 메인 페이지 </summary>
  <p align="center">
      <img src="https://github.com/user-attachments/assets/c85cd65b-6505-4ebd-83ad-fe3504418beb" width="600">
  </p>
  </details>

  <details>
  <summary> &nbsp; 스터디 모집 게시물 상세 조회 </summary>
  <p align="center">
      <img src="https://github.com/user-attachments/assets/a79376ef-4fa2-46a2-9a0c-21dbc3c6c7dd" width="600">
  </p>
  </details>

  <details>
  <summary> &nbsp; 스터디 모집 참가 신청 & 신청 관리 </summary>
  <p align="center">
      <img src="https://github.com/user-attachments/assets/45089934-cb54-487e-8485-0853682f69ae" width="600">
  </p>
  </details>

  <details>
  <summary> &nbsp; 스터디 그룹 생성 </summary>
  <p align="center">
      <img src="https://github.com/user-attachments/assets/5b060cba-6a24-4c91-92b1-69fbac065fb9" width="600">
  </p>
  </details>

  <details>
  <summary> &nbsp; 스터디 그룹 메인 페이지 </summary>
  <p align="center">
      <img src="https://github.com/user-attachments/assets/db4a353b-997a-4d6b-b39c-10fcd8f75275" width="600">
  </p>
  </details>

  <details>
  <summary> &nbsp; 스터디 그룹 게시물 작성 </summary>
  <p align="center">
      <img src="https://github.com/user-attachments/assets/543308f6-bcd3-43c7-a8b0-3860718892a6" width="600">
  </p>
  </details>

<details>
  <summary> &nbsp; 스터디 로드맵 등록 </summary>
  <p align="center">
      <img src="https://github.com/user-attachments/assets/0d4cfac1-6574-4e6d-b60d-0e4ce8fc183b" width="600">
  </p>
  </details>

</details>

<details>
<summary> ⚙️ &nbsp; 마이페이지 </summary>
<div markdown=1>

  <details>
  <summary> &nbsp; 알고리즘 학습 진행율 & 출석 체크 </summary>
  <p align="center">
      <img src="https://github.com/user-attachments/assets/8860eaf2-a025-497a-88ba-f6be487a4f78" width="600">
  </p>
  </details>
</details>

<details>
<summary> 🎯 &nbsp; 신고 및 추천 </summary>
<div markdown=1>

</details>

<br>

### 기술 스택


---

## 2. 설계 문서

### WBS
<details>
  <summary>WBS</summary>
  <img width="1249" height="668" alt="image" src="https://github.com/user-attachments/assets/69008d5d-75f9-49e9-a2b1-df680b0939fd" />

</details>


### 요구사항 명세서
<details>
  <summary>요구사항 명세서</summary>
  <h3>회원 정보</h3>
  <img width="1547" src="https://github.com/user-attachments/assets/4d2ab975-976a-41f7-ba9a-98b45cdeeacc" />

  <h3>알고리즘 학습</h3>
  <img width="1547" src="https://github.com/user-attachments/assets/d1f09441-65bd-4add-8c36-62b8b1c88f76" />

  <h3>기업별 정보 공유</h3>
  <img width="1547" src="https://github.com/user-attachments/assets/4046b165-bf7f-4785-9d59-048b6b2f4fcc" />

  <h3>스터디</h3>
  <img width="1547" src="https://github.com/user-attachments/assets/4feb055e-d148-4dd0-b2d9-3c7f577f45ee" />

  <h3>코딩풀이</h3>
  <img width="1547" src="https://github.com/user-attachments/assets/50027823-c583-47f6-9ec0-6a20a11d04be" />

  <h3>시스템</h3>
  <img width="1547" src="https://github.com/user-attachments/assets/81e31fd9-8878-4fa9-92d8-f31415f6a17e" />

</details>

---

## 3. 와이어 프레임

[🎨 Figma 바로가기](https://www.figma.com/design/HoYo8IPsQSs3pyGbPfF2Kd/Algo?node-id=245-5&t=VxYrlWGrm6QGVVWW-1)

<details>
  <summary>와이어 프레임</summary>
  <h3>메인 화면, 회원가입, 로그인</h3>
<img width="1421" height="519" alt="image" src="https://github.com/user-attachments/assets/16e75fc7-cee7-4758-a41e-7dd87a326410" />
  <h3>알고리즘 학습</h3>
<img width="927" height="511" alt="image" src="https://github.com/user-attachments/assets/5496e146-9d34-455a-bf31-e06bcba51c79" />
  <h3>코딩풀이</h3>
<img width="1142" height="666" alt="image" src="https://github.com/user-attachments/assets/9b13982f-cc1f-401f-ae25-8db0d126e0b1" />
  <h3>기업별 정보 공유</h3>
<img width="1078" height="363" alt="image" src="https://github.com/user-attachments/assets/97ce8eeb-d27a-41c8-a2fc-9148b53cfaa8" />
  <h3>스터디 모집</h3>
<img width="897" height="748" alt="image" src="https://github.com/user-attachments/assets/3e795db5-7a92-4ed0-860b-79a521b15539" />
  <h3>스터디 그룹</h3>
<img width="1053" height="467" alt="image" src="https://github.com/user-attachments/assets/73fc31b5-7c12-494b-8c6e-b811de4b93da" />
  <h3>마이페이지</h3>
<img width="962" height="687" alt="image" src="https://github.com/user-attachments/assets/2b7fc1d1-308b-4dba-a8ad-ab4d07778982" />
<h3>관리자페이지</h3>
<img width="1245" height="669" alt="image" src="https://github.com/user-attachments/assets/046058f5-4b1d-4180-9a6f-f8a40d65bdde" />
</details>

---
## 4. 스토리 보드

<div markdown=1>
  <details>
  <summary> 👤 &nbsp; 로그인 </summary>
  <p align="center">
    <img width="960" height="540" alt="algo-storyboard (5)" src="https://github.com/user-attachments/assets/b71b78b7-4b80-48d1-a56e-6fbcb537ce39" />
  </p>
  </details>


  <details>
  <summary> &nbsp; 📚 알고리즘 개념 학습 </summary>
  <p align="center">
    <img width="960" height="540" alt="algo-storyboard" src="https://github.com/user-attachments/assets/461bc61a-24d3-4b2e-a21b-74058b197c4f" />
  </p>
  </details>

  <details>
  <summary> &nbsp; 🏢 기업정보공유 </summary>
  <p align="center">
    <img width="960" height="540" alt="algo-storyboard (1)" src="https://github.com/user-attachments/assets/1172245a-69bc-41fb-bcc2-f10722976b32" />
  </p>
  </details>

  <details>
  <summary> &nbsp; 🤖 코딩풀이 </summary>
  <p align="center">
    <img width="960" height="540" alt="algo-storyboard (4)" src="https://github.com/user-attachments/assets/d159a589-3fb3-4459-bce3-ed050f99daa1" />
  </p>
  </details>

  <details>
  <summary> &nbsp; 📖 스터디모집 </summary>
  <p align="center">
    <img width="960" height="540" alt="algo-storyboard (3)" src="https://github.com/user-attachments/assets/34a14ccb-b7f4-4c33-9b57-b1308c1fd79a" />
  </p>
  </details>

  <details>
  <summary> &nbsp; 🎒 스터디메인</summary>
  <p align="center">
    <img width="960" height="540" alt="algo-storyboard (2)" src="https://github.com/user-attachments/assets/1cbaebdc-a192-4e7d-b648-36f2d74a757b" />
  </details>

---

## 5. 동료 평가

### 강지륜

| 평가자 | 내용 |
|--------|------|
| 김민지 | 민지님 덕분에 실제 서비스 운영이 가능할 정도로 완벽한 UI 설계를 할 수 있었습니다. 이전까지는 단순 UI 설계만 해왔는데 민지님과 함께하면서 피그마 툴을 배울 수 있었습니다. 또한 백엔드 때와 마찬가지로 꼼꼼한 코드 리뷰를 통해서도 많은 부분을 배웠습니다. 시간이 부족한 상황에서도 발표 당일에 관리자 페이지까지 만들어주셔서 프로젝트 완성도를 높일 수 있었습니다. 또 프로젝트 시작할 때 공통 컴포넌트를 만들자고 제안해주셔서 개발 시간을 크게 단축할 수 있었고 디자인 통일할 수 있어서 정말 감사했습니다! |
| 배태용 | 백엔드 때와 마찬가지로 태용님은 스터디 관련 페이지를 담당하셨습니다. 페이지 수도 많고 직접 구현해야 하는 컴포넌트도 많았음에도 지친 내색 없이 밤새워서라도 끝까지 완성해주셨습니다. 심지어 맡은 작업을 모두 끝낸 뒤에도 남은 페이지들을 먼저 나서서 만들어주시는 모습이 굉장히 인상 깊었고 그런 모습을 보면서 본받고 싶었습니다. 태용님 덕분에 피그마 설계 그대로 예쁜 사이트를 만들 수 있었습니다. 발표도 멋지게 해주셨고 같이 프로젝트를 할 수 있어서 좋았습니다! |
| 정유진 | 마이페이지, 스터디 공간 페이지 구현을 맡아주셨는데 구현해야 될 컴포넌트가 많음에도 불구하고 밤늦게까지 끝까지 만드는 모습 보면서 정말 대단하고 느꼈습니다. 너무 잘 만들어주셔서 실제로 배포해서 제가 직접 사용하고 싶을 정도로 잘 만들어주셨습니다. 예전부터 에디터 기능을 도입해보고 싶었는데 유진님이 이 부분을 구현해주셨습니다. 게시물 작성 시 S3 버킷에 이미지 업로드 기능도 만들어주셔서 게시물에도 이미지 첨부할 수 있게 되었습니다. 함께 프로젝트를 진행할 수 있어서 감사했습니다! |
| 허창훈 | 창훈님과 함께 에러를 찾아서 해결하는 과정이 정말 재밌었습니다. 덕분에 저도 에러 잡으면서 많이 배울 수 있었습니다. 시간이 촉박한 상황에서도 발표 직전까지 밤새워서 어려운 등급업 팝업창 기능을 끝까지 설계해주시는 모습이 인상깊었습니다. 창훈님의 열정적인 태도로 저 역시 더 열심히 개발할 수 있었습니다. 또 프로젝트의 캐릭터인 코알라 이미지를 다양하게 만들어 주셔서 프로젝트 하느라 지친 일상에 큰 힘이 되었습니다. 창훈님 덕분에 재밌게 프로젝트 진행할 수 있어서 감사했습니다~코! |

### 김민지

| 평가자 | 내용 |
|--------|------|
| 강지륜 | 지륜님은 항상 열정적으로 의견을 내주시고 그걸 실제로 구현하는 과정까지 책임감 있게 해주셔서 든든했습니다. 알고리즘 학습 파트 부분은 공통 컴포넌트를 많이 사용하지도 않고 독립적인 느낌이 강했는데 빠르게 구현해주셨습니다. 또한 피그마 디자인 작업 때에도 디자인 개선을 위해 많이 노력해주셔서 감사했습니다. 더미 데이터를 디테일하게 만들어주셔서 프로젝트의 완성도가 더욱 높아질 수 있었던 것 같습니다. |
| 배태용 | 태용님은 가장 처음으로 맡은 부분의 UI 구현을 완료해주셔서 덕분에 많이 참고하고 제 작업에도 적용할 수 있었습니다. 또한 빠르게 작업이 끝난만큼 작업량이 많은 다른 파트 부분의 구현도 나서서 맡아주셔서 프로젝트를 잘 마무리할 수 있었던 것 같습니다. 또한 디자인 설계 과정에서 백엔드 API에 포함되어 있지 않은 컬럼을 추가했는데 힘든 기색 없이 빠르게 백엔드 리팩 작업까지 수행해주셔서 감사했습니다. |
| 정유진 | 유진님은 빠르게 회원가입, 로그인 부분의 UI 구현을 마치고 스터디 부분의 구현을 맡아주셨습니다. 독립적인 느낌이 강한 서비스라 공통 컴포넌트를 많이 사용하지 않아 새로 구현해야 했던 컴포넌트가 많았는데 밤을 새워가며 열심히 작업해주셔서 완료해주신 점이 인상깊었습니다. 또한 백엔드 피드백 받았던 redis 부분도 빠르게 리팩해주셔서 더욱 완성도가 높아졌던  것 같습니다. |
| 허창훈 | 창훈님은 항상 힘든 기색 없이 늦게까지 작업하면서 맡은 부분 구현을 완료해주셨습니다. 프론트 작업을 시작할 때 먼저 서치해서 사용을 제안해주신 덕분에 편하게 작업할 수 있었습니다. 또한 프로젝트의 캐릭터로 다양한 이미지를 만들어내주셔서 즐거운 분위기를 만들어주셨습니다. 시간이 촉박한 상황에서 프로젝트의 완성도를 높이기 위해 pinia를 적용해 등급업 기능을 완성시키신 부분이 인상깊었습니다. |

### 배태용

| 평가자 | 내용 |
|--------|------|
| 강지륜 | 백엔드 프로젝트에 이어 꼼꼼한 PR 확인 덕분에 프론트엔드 프로젝트도 더 편하게 수정하거나 개발 할 수 있었고,  댓글쪽 기능에서 계속해서 버그 때문에 제대로 작동 안하는 일이 있었는데, 끝까지 포기하지 않고 개발해서 수정하는 모습이 개발자로써 상당한 자극을 받을 수 있었습니다. 그리고 Figma를 작업할 때도 못생긴 제 UI 설계를 엄청나게 이쁜 UI로 재단장 해주시는 등 많은 도움을 주셔서 대단하다고 생각했습니다. |
| 김민지 | 지난 프로젝트때나 이번 프로젝트때나 엄청난 작업량을 먼저 맡아서 해주시는게 정말 대단하다고 느꼈습니다. 기본적으로 팀의 방향성을 잡아주시면서 이끌어주셔서 믿고 프로젝트를 진행하였습니다. 잠은 언제 주무시는지 궁금할 정도로 늘 프로젝트 작업을 진행해주셨고, 특히 Figma 작업을 할 때 처음 써보는 팀원들을 위해서 기능 사용방법들을 동영상으로 녹화해서 공유해주신 덕에 더 쉽게 멋있는 UI 설계를 할 수 있었습니다. |
| 정유진 | 이번 프론트엔드 프로젝트에서 회원 관련 기능(회원가입, 로그인)과 스터디 쪽을 맡으셔서 남들보다 조금 더 많은 분량을 작업하셨는데, 잠은 도대체 언제 주무시는지 모를 정도로 매번 기한을 맞춰서 작업을 해주시는 걸 보고 정말 대단하다고 느꼈습니다. 자신이 맡은 파트는 책임감을 가지고 작업을 해주시고, 작업하다 모르는게 있을 때 물어보면 언제나 친절하게 답변을 해주셔서 프로젝트 내내 편하게 작업을 할 수 있었던 것 같습니다.|
| 허창훈 | 창훈님은 이번에도 든든하게 프로젝트 개발을 해주셨습니다. 공통적으로 사용되는 컴포넌트를 만들 때 그 종류가 생각보다 다양해서 꽤나 고생해주셨는데, 덕분에 프론트엔드 작업을 하는 동안 편하게 작업을 할 수 있었던 것 같습니다. 그 외에도 프로젝트의 트레이드 마크인 캐릭터 디자인을 범용성 있게 만들어주셔서, 덕분에 분위기도 환기해가면서 재밌게 프로젝트를 진행할 수 있었습니다. 게다가 하나에 꽂히시면 끝까지 파고들어서 해결시키는 모습을 보고 배워야겠다는 생각도 들었습니다. |

### 정유진

| 평가자 | 내용 |
|--------|------|
| 강지륜 | 알고리즘 학습 페이지를 피그마 시안보다 더 완성도 높게 구현해주셨고, 더미데이터를 실제 데이터처럼 꼼꼼하게 구성해주셔서 매우 인상 깊었습니다. 맡은 일을 가장 먼저 마감 기한 내에 완수하시고, 다른 팀원들의 작업도 적극적으로 도와주셔서 큰 도움이 되었습니다. 또한 어려운 문제에 부딪혔을 때도 포기하지 않고 끈기 있게 도전하며 끝내 해결해내는 모습이 정말 멋졌습니다.|
| 김민지 | 피그마 디자인을 매우 정교하게 완성해주신 덕분에 프로젝트의 비주얼 퀄리티가 매우 높아진 것 같습니다. 또한 팀원들의 진행 상황을 꾸준히 파악하며 해야 할 일들을 잘 정리해주셔서 저 역시 제 역할에 집중할 수 있었습니다. MSA 및 백엔드 로직을 꼼꼼하게 수정하고, 발표 직전에는 관리자 페이지까지 구현해주셔서 프로젝트의 완성도와 안정성이 크게 향상된 것 같습니다.|
| 배태용 | 프로젝트를 진행하면서 스터디 관련 기능의 수정이나 추가가 필요할 때마다 적극적으로 백엔드 로직을 개선해주셔서 프론트엔드 구현을 훨씬 빠르게 진행할 수 있었습니다. 또한 스터디 모집 페이지를 높은 완성도로 구현해주셔서 프로젝트의 전체적인 완성도를 높이는 데 큰 기여를 하셨습니다. 마지막에는 시간이 부족해 어려울 줄만 알았던 마이페이지의 메인 페이지까지 구현해주셨고, 필요한 부분을 스스로 찾아 추가하며 끝까지 최선을 다하는 모습이 매우 인상 깊었습니다. |
| 허창훈 | 백엔드에서 맡으셨던 코딩풀이 기능을 프론트엔드에서도 직접 구현하시며 여러 어려움을 겪으셨지만, 새벽까지 끈기 있게 작업해주신 덕분에 프로젝트의 핵심 기능인 AI 피드백을 성공적으로 완성할 수 있었습니다. 더불어 저희 프로젝트의 컨셉에 맞는 다양한 코알라 이미지를 직접 생성해주셔서 프로젝트의 컨셉이 한층 뚜렷해졌고, 팀의 분위기 역시 긍정적으로 이끌어주셨습니다!|

### 허창훈

| 평가자 | 내용 |
|--------|------|
| 강지륜 | 지륜님은 함께 일하면서 정말 든든한 동료라는 생각이 많이 들었습니다. 항상 팀 전체의 상황을 살피며, 누가 어려움을 겪고 있으면 먼저 다가가 도와주는 모습이 인상 깊었습니다. 특히 댓글 기능 관련해서 끝까지 문제를 포기하지 않고 해결해내는 과정에서,함께 고민하고 소통하는 지륜님의 끈기와 책임감을 많이 느꼈습니다. 코드 리뷰나 의 시간에도 늘 차분하고 논리적으로 의견을 나눠주셔서 대화가 정말 편했습니다. 지륜님 덕분에 팀 전체가 더 단단하게 연결되어 있었다고 생각합니다. 같이 프로젝트를 할 수 있어서 정말 감사했고, 다음에도 꼭 함께하고 싶은 동료예요~코!🐨|
| 김민지 | 민지님은 이번 프로젝트의 중심이자 팀을 이끄는 힘이 되어주셨습니다. 프로젝트 초반부터 공통 컴포넌트 제안, 디자인 가이드 제작 등 모두가 같은 방향을 바라보며 작업할 수 있게 세심하게 챙겨주신 덕분에 정말 편하게 협업할 수 있었습니다. 또한 각자 의견을 존중해주시면서도 정리력 있게 방향을 잡아주셔서 회의 때마다 자연스럽게 팀 전체가 한마음으로 움직일 수 있었습니다. 저를 비롯한 몇몇 팀원분들은 피그마를 처음 쓰는데 직접 영상을 만들어주셔서 가이드를 해 주신 점에 대해 감동이었습니다. 민지님 덕분에 협업이 ‘편하다’는 게 어떤 건지 몸소 느낄 수 있었어요. 진심으로 존경스럽고, 함께해서 너무 감사했습니다~코!🐨|
| 배태용 | 태용님은 묵묵하지만 늘 확실하게 팀을 지탱해주는 분이었습니다. 항상 맡은 일은 책임감 있게 마감 기한보다 빠르게 끝내주시고, 그 후에는 다른 팀원들의 진행 상황을 살펴보며 먼저 도와주시는 모습이 정말 인상 깊었습니다. 작업 중 막히는 부분이 있으면 부담 주지 않게 자연스럽게 같이 고민해주셔서 마음이 편했습니다. 코드 리뷰에서도 피드백이 늘 따뜻하면서도 실질적인 도움이 되었고, 프로젝트 전체의 완성도를 생각하면서 행동하시는 모습에 진심으로 존경심이 들었습니다. 태용님과 함께라서 끝까지 안정적으로 프로젝트를 마무리할 수 있었습니다. 정말 감사했습니다~코!🐨|
| 정유진 | 유진님은 언제나 밝고 긍정적인 에너지로 팀 분위기를 이끌어주셨습니다. 항상 커뮤니케이션이 빠르고 정확해서, 함께 협업할 때 불편함이 한 번도 없었습니다. 본인의 파트뿐 아니라 다른 부분과의 연동까지 꼼꼼하게 챙겨주시고, 필요할 땐 먼저 연락해서 조율해주시는 모습이 너무 멋졌습니다. 특히 로그인·회원 기능처럼 복잡한 구조를 구현하면서도, 늘 차분하게 문제를 정리하고 공유해주셔서 팀 전체가 한결 편하게 일할 수 있었습니다. 유진님 덕분에 팀 분위기가 늘 따뜻하고 안정적이었고, 함께 일하며 진심으로 배울 점이 많았습니다. 정말 감사했습니다~코!🐨|
